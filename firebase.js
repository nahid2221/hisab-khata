...
if (!configured) {
  window.HKCloud.ready = Promise.resolve(null);
} else {
  ...
  window.HKCloud.ready = (async () => {
    ...
    window.HKCloud.auth = auth;
    window.HKCloud.db = db;

    window.HKCloud.GoogleAuthProvider = GoogleAuthProvider;

    window.HKCloud.signInWithPopup = signInWithPopup;

    window.HKCloud.signInWithEmailAndPassword =
      signInWithEmailAndPassword;

    window.HKCloud.createUserWithEmailAndPassword =
      createUserWithEmailAndPassword;

    window.HKCloud.signOut = signOut;

    window.HKCloud.onAuthStateChanged =
      onAuthStateChanged;

    window.HKCloud.load = async (uid) => {
    ...
    window.HKCloud.save = async (uid, state) => {
    ...
    onAuthStateChanged(auth, (user) => {
      ...
      window.HKCloud.user = user;

      window.dispatchEvent(
        new CustomEvent('hk-auth', {
          detail: user
        })
      );
